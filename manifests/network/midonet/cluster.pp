...
#   Defaults to hiera('is_mem', undef)
#
# [*keystone_host*]
...
#   Defaults to hiera('midonet_cluster_keystone_tenant_name', undef)
#
# [*keystone_protocol*]
#   (Optional) Protocol ("http" or "https") to make the keystone requests.
#   Defaults to hiera('midonet_cluster_keystone_protocol', undef)
#
# [*keystone_admin_token*]
#   (Optional) Keystone token for the admin user.
#   Defaults to hiera('midonet_cluster_keystone_admin_token', undef)
#
# [*keystone_domain_name*]
#   (Optional) Keystone domain where the user has enough rights.
#   Defaults to hiera('midonet_cluster_keystone_domain_name', undef)
#
# [*keystone_domain_id*]
...
#   Defaults to hiera('midonet_cluster_keystone_version', undef)
#
# [*ssl_keystore_path*]
#   (Optional) Path where the SSL keystore is.
#   Defaults to hiera('midonet_cluster_ssl_keystore_path', undef)
#
# [*ssl_keystore_pwd*]
#   (Optional) Password for the SSL keystore.
#   Defaults to hiera('midonet_cluster_ssl_keystore_pwd', undef)
#
# [*nsdb_nodes*]
...
#   Default: hiera('midonet_cluster_cluster_api_port', undef)
#
# [*elk_cluster_name*]
#   (Optional) Elasticsearch cluster name. Not needed if running a single
#   ELK node.
#   Default: hiera('midonet_cluster_Optional', undef)
#
# [*elk_target_endpoint*]
#   (Optional) Target ELK endpoint to be configured.
#   Default: hiera('midonet_cluster_elk_target_endpoint', undef)
#
# [*endpoint_host*]
#   (Optional) Host where the unified endpoint will be bound.
#   Default: hiera('midonet_cluster_endpoint_host', undef)
#
# [*endpoint_port*]
#   (Optional) Port that is going to be bound.
#   Default: hiera('midonet_cluster_endpoint_port', undef)
#
# [*ssl_source_type*]
#   (Optional) SSL source type. Choose between 'autosigned' , 'keystore'
#   or 'certificate'.
#   Default: hiera('midonet_cluster_ssl_source_type', undef)
#
# [*ssl_cert_path*]
#   (Optional) Path for the SSL certificate.
#   Default: hiera('midonet_cluster_ssl_cert_path', undef)
#
# [*ssl_privkey_path*]
#   (Optional) Path for the private SSL key.
#   Default: hiera('midonet_cluster_ssl_privkey_path', undef)
#
# [*ssl_privkey_pwd*]
#   (Optional) SSL private key password.
#   Default: hiera('midonet_cluster_ssl_privkey_pwd', undef)
#
# [*flow_history_port*]
#   (Optional) Port to be used by the flow history endpoint.
#   Default: 'hiera('midonet_cluster_flow_history_port', undef)
#
# [*jarvis_enabled*]
#   (Optional) Whether to enable Jarvis or not.
#   Default: 'hiera('midonet_cluster_jarvis_enabled', undef)
#
# [*state_proxy_address*]
#   (Optional) Address that the state proxy service will bind.
#   Default: hiera('midonet_cluster_state_proxy_address', undef)
#
# [*state_proxy_port*]
#   (Optional) Port to be used by the state proxy.
#   Default: hiera('midonet_cluster_state_proxy_port', undef)
#
# [*step*]
...
class tripleo::network::midonet::cluster (
  ...
  $keystone_tenant_name      = hiera('midonet_cluster_keystone_tenant_name', undef),
  ...
  $keystone_domain_name      = hiera('midonet_cluster_keystone_domain_name', undef),
